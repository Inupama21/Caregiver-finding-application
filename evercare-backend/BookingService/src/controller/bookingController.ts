...
import { createBookingNotification } from "./notificationController";
...
export const updateBookingStatus = async (
  ...
  try {
    ...
    // Create notification for careseeker when booking is accepted or rejected
    if (status === "accepted" || status === "rejected") {
      try {
        const message = status === "accepted" 
          ? `Great news! ${booking.caregiverName} has accepted your booking request. You can now proceed with payment.`
          : `Unfortunately, ${booking.caregiverName} has declined your booking request. You can try booking with another caregiver.`;
        
        const notificationType = status === "accepted" ? "booking_accepted" : "booking_rejected";
        
        await createBookingNotification(bookingId, notificationType, message);
